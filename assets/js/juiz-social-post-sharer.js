	...
	if ( $('.juiz_sps_link_mail').length ) {
		...
		$('.juiz_sps_link_mail').find('a').on('click', function(){
							...
							'<div class="juiz-sps-modal-content">' +
								'<form id="jsps-email-form" name="jsps-email" action="' + jsps.modalEmailAction + '" method="post">' +
									...
									'<p class="juiz-sps-textarea-line">' + 
										'<label for="jsps-message">' + jsps.modalEmailMessage + ' <small>(' + jsps.modalEmailOptional + ')</small></label>' +
										'<textarea id="jsps-message" name="jsps-message" cols="50" rows="7" aria-required="false"></textarea>' +
									'</p>' +
									'<p class="juiz-sps-message-info"> ' + jsps.modalEmailInfo + '<br>' + jsps.modalEmailMsgInfo + '</p>' +
			...
			$('body').on( 'submit.jsps', '#jsps-email-form', function(e){
				...
				console.log(jsps.ajax_url, to_send);
